import type { FormSchema, FieldDef, FieldGroup, ObjectFieldGroup, ArrayFieldGroup } from '@/types'
...
export function isItemCompleted(schema: FormSchema, data: Record<string, any>): boolean {
  const fields = getFormFields(schema)
...
export function isListItemCompleted(fields: FieldDef[], data: Record<string, any>): boolean {
  // 检查必填项
  const requiredFields = fields.filter(f => f.required)
  for (const field of requiredFields) {
    const value = data[field.key]
