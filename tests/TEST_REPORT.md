...
| 模块层                   | 测试文件数 | 用例数  | 占比     |
| ------------------------ | ---------- | ------- | -------- |
| 引擎层 (Engines)         | 5          | 45      | 31.0%    |
| 状态管理层 (Stores)      | 4          | 38      | 26.2%    |
| 组合式函数 (Composables) | 5          | 38      | 26.2%    |
| 数据库层 (Database)      | 1          | 10      | 6.9%     |
| 工具层 (Utils)           | 1          | 18      | 12.4%    |
| **合计**                 | **16**     | **145** | **100%** |
...
### 2.5 工具层 (Utils) — 1 文件 / 18 用例 / ✅ 全部通过

#### html-download.test.ts (18 用例)

| 用例 | 描述                              | 结果 |
| ---- | --------------------------------- | ---- |
| 1    | beautifyHtml 格式化简单 HTML      | PASS |
| 2    | beautifyHtml 正确处理嵌套标签缩进 | PASS |
| 3    | beautifyHtml 处理自闭合标签       | PASS |
| 4    | beautifyHtml 格式化 style 标签    | PASS |
| 5    | beautifyHtml 保留 HTML 注释       | PASS |
| 6    | beautifyHtml 处理 DOCTYPE         | PASS |
| 7    | beautifyHtml 处理带属性的标签     | PASS |
| 8    | beautifyHtml 处理空 HTML          | PASS |
| 9    | beautifyHtml 正确缩进多层级嵌套   | PASS |
| 10   | beautifyHtml 处理 script 标签     | PASS |
| 11   | minifyHtml 移除 HTML 注释         | PASS |
| 12   | minifyHtml 移除多余空白           | PASS |
| 13   | minifyHtml 压缩标签间空白         | PASS |
| 14   | minifyHtml 保留属性值中的空格     | PASS |
| 15   | minifyHtml 移除换行符             | PASS |
| 16   | minifyHtml 清理属性间多余空格     | PASS |
| 17   | downloadHtml 创建正确格式的 Blob  | PASS |
| 18   | downloadHtmlPackage 生成 zip 包   | PASS |

**覆盖函数**: `beautifyHtml`, `minifyHtml`, `downloadHtml`, `downloadHtmlPackage`
...
| `src/utils/html-download.ts`             | 高       | 100%     | 高       | ~95%       |
...
_本报告由 Vitest v2.1.9 生成于 2026-04-05T20:16 UTC+8_
